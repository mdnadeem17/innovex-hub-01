...
const ProjectMemories = () => {
    ...
    const memories = useQuery(api.memories.list, projectId ? { project_id: projectId } : "skip") || [];
    ...
    const handleDelete = async (e: React.MouseEvent, id: Id<"memories">) => {
        e.stopPropagation();
    ...
    if (!project) return <div className="min-h-screen pt-24 text-center text-white">Loading Project...</div>;

    return (
        <div className="min-h-screen bg-background text-white pt-20 pb-12">
            {/* Header */}
            <div className="relative h-[40vh] overflow-hidden">
                <ImageWithFallback src={project.image_url} alt={project.title} className="w-full h-full object-cover opacity-60" />
                ...
                <div className="absolute bottom-0 left-0 right-0 p-8 container mx-auto">
                    ...
                    >
                        {project.title} <span className="text-primary font-light italic">Journey</span>
                    </motion.h1>
                    <p className="max-w-2xl text-lg text-muted-foreground line-clamp-2">{project.description}</p>
                </div>
            </div>
...
export default ProjectMemories;
