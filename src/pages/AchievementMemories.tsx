...
import AddAchievementMemoryModal from "@/components/AddAchievementMemoryModal";
...
const AchievementMemories = () => {
    ...
    const memories = useQuery(api.achievement_memories.list, achievementId ? { achievement_id: achievementId } : "skip") || [];
    ...
    const deleteMemoryMutation = useMutation(api.achievement_memories.deleteMemory);

    const handleDelete = async (e: React.MouseEvent, id: Id<"achievement_memories">) => {
        e.stopPropagation();
    ...
    if (!achievement) return <div className="min-h-screen pt-24 text-center text-white">Loading Achievement...</div>;

    return (
        <div className="min-h-screen bg-background text-white pt-20 pb-12">
            {/* Header */}
            <div className="relative h-[40vh] overflow-hidden">
                <ImageWithFallback src={achievement.image_url} alt={achievement.title} className="w-full h-full object-cover opacity-60" />
                <div className="absolute inset-0 bg-gradient-to-t from-background via-background/60 to-transparent" />
                <div className="absolute bottom-0 left-0 right-0 p-8 container mx-auto">
                    <button
                        onClick={() => navigate(`/achievements/${achievementId}`)}
                    ...
                    >
                        {achievement.title} <span className="text-primary font-light italic">Journey</span>
                    </motion.h1>
                    <p className="max-w-2xl text-lg text-muted-foreground line-clamp-2">{achievement.description}</p>
                </div>
            </div>
