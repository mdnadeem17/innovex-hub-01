...
interface AddparticipationMemoryModalProps {
    isOpen: boolean;
    onClose: () => void;
    participationId: Id<"participations">;
}
...
const AddparticipationMemoryModal = ({ isOpen, onClose, participationId }: AddparticipationMemoryModalProps) => {
    ...
    const handleSubmit = async () => {
        if (!title || imageFiles.length === 0 || !category) {
        ...
        try {
            await Promise.all(
                imageFiles.map(async (file) => {
                    ...
                    const storageId = await uploadImage(file, postUrl);
                    await createMemory({
            ...
            toast({ title: "Success", description: "Memories added successfully!" });
            onClose();
            // Reset form
            setTitle("");
            setDescription("");
...
export default AddparticipationMemoryModal;
