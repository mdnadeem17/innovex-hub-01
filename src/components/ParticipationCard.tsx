    ...
    >
      ...
      <div className="p-5 flex flex-col flex-grow">
        <div className="flex justify-between items-start mb-2">
            <h3 className="font-display text-sm tracking-wider text-primary">{participation.title}</h3>
            <span className="text-xs text-muted-foreground bg-white/5 px-2 py-1 rounded border border-white/10 whitespace-nowrap ml-2">
                {participation.date}
            </span>
        </div>
        <p className="text-xs text-accent font-display tracking-wider mb-2">{participation.event_name}</p>
        <p className="text-sm text-muted-foreground line-clamp-2 mb-4 flex-grow">{participation.description}</p>
